...
sub do_nm
{
	...
	if (-e "$source.c" || -e "$source.S") {
		$source = "$objtree$File: //depot/swdev/LAN/Switch/KSZ/kernels/linux-3.18/scripts/namespace.pl $source";
	} else {
		$source = "$srctree$File: //depot/swdev/LAN/Switch/KSZ/kernels/linux-3.18/scripts/namespace.pl $source";
	}
	if (! -e "$source.c" && ! -e "$source.S") {
